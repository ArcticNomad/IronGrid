  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        const response = await fetch(`/profile?user_id=${encodeURIComponent(user_id)}`);
    ...
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold">{formatDate(profile.registration_date)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {profile.account_type === 'trainer' && profile.trainer_info && (
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="space-y-3">
              <p><strong>Specialization:</strong> {profile.trainer_info.specialization}</p>
              <p><strong>Certification:</strong> {profile.trainer_info.certification}</p>
              <p><strong>Experience:</strong> {profile.trainer_info.years_of_experience} years</p>
              <p><strong>Hourly Rate:</strong> ${profile.trainer_info.hourly_rate}</p>
            </div>
          </div>
        )}
          ...
          <>
            {profile.member_info && (
              <div className="bg-white p-6 rounded-lg shadow">
                ...
                <div className="space-y-3">
                  <p><strong>Fitness Goals:</strong> {profile.member_info.fitness_goals}</p>
                  <p><strong>Current Weight:</strong> {profile.member_info.current_weight} kg</p>
                  <p><strong>Target Weight:</strong> {profile.member_info.target_weight} kg</p>
                  <p><strong>Height:</strong> {profile.member_info.height} cm</p>
                </div>
              </div>
            )}
