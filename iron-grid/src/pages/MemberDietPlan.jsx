...
const MemberDietPlan = ({ memberId }) => {
  ...
  if (loading) return <div className="text-white text-center py-8">Loading diet plan...</div>;
  if (error) return <div className="text-red-500 text-center py-8">Error: {error}</div>;
  if (!dietPlan) return <div className="text-white text-center py-8">No diet plan assigned</div>;
    ...
    <div className="bg-gray-800 rounded-lg shadow-md p-6 text-white">
      ...
      <div className="mb-4">
        <p className="text-gray-300">
          Daily Calories: {dietPlan.daily_calories} kcal • Cuisine: {dietPlan.cuisine_preferences}
        </p>
        {dietPlan.notes && (
      ...
      <div className="space-y-6">
        {[1, 2, 3, 4, 5, 6, 7].map(dayNum => {
          const dayMeals = dietPlan.meals?.filter(meal => meal.day_num === dayNum) || [];
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                          <div>
                            <span className="text-gray-400">Calories:</span> {meal.calories}
                          </div>
                          <div>
                            <span className="text-gray-400">Protein:</span> {meal.protein}g
                          </div>
                          <div>
                            <span className="text-gray-400">Carbs:</span> {meal.carbs}g
                          </div>
                          <div>
                            <span className="text-gray-400">Fat:</span> {meal.fat}g
                          </div>
                        </div>
                      </div>
                    </div>
