...
const MemberWorkoutPlan = ({ memberId }) => {
  ...
  if (loading)
    return (
      <div className="text-white text-center py-8">Loading workout plan...</div>
    );
  if (error)
    ...
    <div className="bg-gray-800 rounded-lg shadow-md p-6 text-white">
      ...
      <div className="mb-4">
        <p className="text-gray-300">
          Duration: {workoutPlan.duration_weeks} weeks •{" "}
          {workoutPlan.duration_session} minutes per session
        </p>
        {workoutPlan.notes && (
      ...
      <div className="space-y-6">
        {[1, 2, 3, 4, 5, 6, 7].map((dayNum) => {
          const dayExercises =
            workoutPlan.exercises?.filter((ex) => ex.day_num === dayNum) || [];
